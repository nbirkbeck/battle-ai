...
inline bool IsValidPoint(const nacb::Vec3d& x) {
  return (2 * x.x == int(2 * x.x)) && (x.y == int(x.y)) &&
         (2 * x.z == int(2 * x.z));
}
...
public:
  ...
  bool LineHits(const nacb::Vec3d& p1, const nacb::Vec3d& p2, double* t,
                Agent** agent) const;
  ...
  const std::vector<std::unique_ptr<PowerUp>>& power_ups() const {
    return power_ups_;
  }
  ...
  const AccessibilityMap& access_map() const { return *access_map_.get(); }
