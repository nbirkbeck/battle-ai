...
class Projectile {
 public:
  Projectile(const nacb::Vec3d& p,
             const nacb::Vec3d& v,
             double mass,
             double damage) : p_(p), v_(v), mass_(mass), damage_(damage), is_active_(true) {
  }
...
class Weapon {
 public:
  Weapon(double damage = 5, double firing_rate = 5,
         double speed = 200, double ammo = 100) :
