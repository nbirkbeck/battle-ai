...
public:
  GLOgreWindow() : keyboard_(nullptr), mouse_(nullptr)
  {
    ...
    Ogre::ResourceGroupManager::getSingleton().addResourceLocation("/usr/share/OGRE/media/packs", "FileSystem");
    ...
    Ogre::ResourceGroupManager::getSingleton().addResourceLocation("/usr/share/OGRE/media/packs/SdkTrays.zip", "Zip", "Font");

    Ogre::ResourceGroupManager::getSingleton().addResourceLocation("levels", "FileSystem");
    ...
    // Load the font (fail early if it isn't loaded)
    /*
    Ogre::FontPtr mFont = Ogre::FontManager::getSingleton().getByName("SdkTrays/Value", "Font");
    //mFont->setSource("Vera.ttf");
    //mFont->setSize(15);
    //mFont->setResolution(96);
    std::cerr << "Font:" << mFont.get() << std::endl;
    std::cerr << "Source:" << mFont->getSource() << std::endl;
    mFont->load();
    */
    scnMgr->addRenderQueueListener(overlaySystem_);
  ...
  int width() override { return 1280; }
  int height() override { return 720; }
