...
Agent* CreateAgent(int i, const nacb::Vec3d& pos, const nacb::Quaternion& quat,
...
public:
  Win(const std::string& level_mesh, const std::string& level_texture)
      : GLWindow(1920 / 2, 1080 / 2) {

    glEnable(GL_MULTISAMPLE);
    glHint(GL_MULTISAMPLE_FILTER_HINT_NV, GL_NICEST);

    world_renderer_.reset(new WorldRenderer(
        "/usr/share/fonts/bitstream-vera/Vera.ttf", level_mesh, level_texture));
    setRefreshRate(60);
  ...
  void drawScene() override {
    ...
    for (int i = 0; i < 360; i += 1) {
      nacb::Vec3d d =
        nacb::Quaternion::rod(nacb::Vec3d(0, 1 * double(i) / 360 * 2.0 * M_PI,
    0)).rotate(nacb::Vec3d(0, 0, 1)); nacb::Vec3d p2 = user_agent->pos() + d *
    50; std::vector<const Agent*> agents; Agent* agent; double t = 0;
