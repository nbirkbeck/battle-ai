...
public:
  ...
  void InitOverlay() {
    ...
    overlay_container_ =
        (Ogre::OverlayContainer*)Ogre::OverlayManager::getSingleton()
            .createOverlayElement("Panel", "InfoOverlayPanel");
  ...
  void InitScene(const std::string& level_mesh) {
    ...
    for (const auto& agent : world_->agents()) {
      ...
      Ogre::SceneNode* body_node =
          scnMgr->getRootSceneNode()->createChildSceneNode(name);
      body_node->attachObject(body_entity);

      Ogre::Entity* head_entity = scnMgr->createEntity("Head.mesh");
      body_node->createChildSceneNode(std::string(name) + ".Head")
          ->attachObject(head_entity);

      Ogre::Entity* right_wheel = scnMgr->createEntity("RightWheel.mesh");
      body_node->createChildSceneNode(std::string(name) + ".RightWheel")
          ->attachObject(right_wheel);

      Ogre::Entity* left_wheel = scnMgr->createEntity("LeftWheel.mesh");
      body_node->createChildSceneNode(std::string(name) + ".LeftWheel")
          ->attachObject(left_wheel);
  ...
  void UpdateOverlayElements() {
    ...
    for (int i = 0; i < (int)world_->agents().size(); ++i) {
      ...
      const Ogre::Vector3 p(agent->pos().x, agent->pos().y + 1.8,
                            agent->pos().z);
      const auto pt =
          camera_->getProjectionMatrix() * (camera_->getViewMatrix() * p);
    ...
    Ogre::OverlayElement* l =
        OverlayManager::getSingleton().createOverlayElement("TextArea",
                                                            "Stats");
  ...
  void UpdateScene() {
    ...
    if (animating_ && first_person_ && user_agent_) {
      ...
      cquat =
          user_agent_->quat() * nacb::Quaternion::rod(nacb::Vec3d(0, M_PI, 0));
      SyncCamera();
    }
    ...
    for (int i = 0; i < (int)world_->agents().size(); ++i) {
      ...
      const auto& agent = world_->agents()[i];
      agents_[i]->setPosition(world_->agents()[i]->pos().x, 0,
      ...
      const nacb::Quaternion q2 =
          q.conj() * nacb::Quaternion::rod(nacb::Vec3d(0, a2, 0));
      agents_[i]
          ->getChild(agents_[i]->getName() + ".Head")
          ->setOrientation(q2.a, q2.v.x, q2.v.y, q2.v.z);
    }
    ...
    if (world_->projectiles().size()) {
      std::cout << "Num projectiles:" << world_->projectiles().size() << " "
                << projectiles_.size() << std::endl;
    }
    projectiles_.clear();
    ...
    for (const auto& proj : world_->projectiles()) {
      ...
      const nacb::Quaternion q =
          nacb::Quaternion::rod(nacb::Vec3d(0, theta, 0));
  ...
  void SetUserAgent(UserAgent* user_agent) { user_agent_ = user_agent; }

  void SetInfoString(const std::string str) { info_string_ = str; }
