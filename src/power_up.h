...
class PowerUp {
public:
  PowerUp(const nacb::Vec3d pos, const state::PowerUp::Type& type,
          double amount, double respawn_delay = 30)
      : pos_(pos), type_(type), amount_(amount), respawn_delay_(respawn_delay),
        time_until_respawn_(0) {}

  void Give(Agent* agent);

  void Reset() { time_until_respawn_ = 0; }

  void Step(double dt) {
    time_until_respawn_ = std::max(0.0, time_until_respawn_ - dt);
  }

  bool IsActive() const { return time_until_respawn_ <= 0.0; }
  ...
  double time_until_respawn() const { return time_until_respawn_; }

  state::PowerUp::Type type() const { return type_; }

  double amount() const { return amount_; }

  double respawn_delay() const { return respawn_delay_; }
