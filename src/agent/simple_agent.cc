...
std::deque<nacb::Vec3d> SmoothPoints(std::deque<nacb::Vec3d> points) {
  ...
  for (int iter = 0; iter < 2; ++iter) {
    ...
    for (int i = 1; i < (int)output_points.size() - 1; ++i) {
      output_points[i] =
          (points[i - 1] + 2 * points[i] + points[i + 1]) * (1.0 / 4.0);
    }
  }
  return output_points;
}

bool Plan::IsPathObstructedByPoints(
    const std::vector<nacb::Vec3d>& other_points, double limit) const {
  if (waypoints_.empty())
    return false;
...
void PlanFollowingAgent::GetActions(const ObservableState& state,
  ...
  if (state.visible_agents.size() > 0) {
    for (const auto& agent : state.visible_agents) {
      ...
      if (agent.confidence < shoot_confidence_)
        continue;
      ...
      const nacb::Quaternion q =
          nacb::Quaternion::rod(nacb::Vec3d(0, angle, 0)) *
...
void SimpleAgent::GetActions(const ObservableState& state,
  ...
  const bool path_obstructed =
      plan_ && plan_->IsPathObstructedByPoints(
                   other_points, params_.replan_obstruction_distance());
  if (path_obstructed) {
  ...
  if (!plan_ || should_replan || path_obstructed) {
    ...
    if (urand() < params_.favor_powerups()) {
      for (const auto& pup : state.power_ups) {
        if (armor_deficit == 0 && pup->type() == state::PowerUp::POWER_UP_ARMOR)
          continue;
        if (health_deficit == 0 &&
            pup->type() == state::PowerUp::POWER_UP_HEALTH)
          continue;

        std::deque<nacb::Vec3d> points;
        double pup_distance =
            Search(state.access_map, other_points, pos(), pup->pos(), &points);
        if (pup_distance < 1 && pup->time_until_respawn() > 20)
          continue;
        double score = pup_distance + pup->time_until_respawn() +
    ...
    if ((good_score > 5 && good_dist <= 1)) {
        ...
        for (const auto& agent : state.visible_agents) {
          ...
          for (int x = -search_zone; x <= search_zone; ++x) {
            for (int y = -search_zone; y <= search_zone; ++y) {
              ...
              if (c.x < 0 || c.y < 0 || c.x >= access_map->map().w ||
                  c.y >= access_map->map().h) {
                continue;
              }
              if (access_map->map()(c.x, c.y)) {
                ...
                candidates.push_back(std::make_pair(
                    (pt - agent.pos).dot(dir) + (rand() % 2), pt));
              }
            }
          }
          if (rand() % state.visible_agents.size() == 0)
            break;
        }
      } else {
          ...
          for (int y = 0; y < access_map->map().h; ++y) {
            if (access_map->map()(x, y)) {
              ...
              candidates.push_back(
                  std::make_pair((pt - pos()).len() + urand(-3, 3), pt));
            }
          }
        }
      }
      ...
      std::sort(candidates.begin(), candidates.end(),
                [](const std::pair<double, nacb::Vec3d>& p1,
                   const std::pair<double, nacb::Vec3d>& p2) {
                  return p1.first > p2.first;
                });
      VLOG(3) << candidates.front().first << " " << candidates.back().first;
      Search(state.access_map, other_points, pos(), candidates[0].second,
             &good_points);
    }
